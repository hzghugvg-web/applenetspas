-- Trigger: cascade delete issued_configs by upstream_url when vless_links row is deleted
...
AS $$
BEGIN
  DELETE FROM public.issued_configs WHERE upstream_url = OLD.url;
  RETURN OLD;
END $$;

DROP TRIGGER IF EXISTS trg_cleanup_issued_on_link_delete ON public.vless_links;
CREATE TRIGGER trg_cleanup_issued_on_link_delete
BEFORE DELETE ON public.vless_links
FOR EACH ROW EXECUTE FUNCTION public.cleanup_issued_on_link_delete();

-- Fix subscription date logic: set to now()+30d instead of accumulating
